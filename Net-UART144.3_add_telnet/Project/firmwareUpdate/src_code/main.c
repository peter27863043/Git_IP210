...
*               
*******************************************************************************/
u16_t reboot_check1 _at_ 0x7ffe;
u16_t reboot_check2 _at_ 0x7ffc;
void main (void)
{	
	u8_t chk_tmp, count,key;
...
//sorbica    WDTEN=0;//disable watch dog timer
	...
	TRACE_TEST(("\n->Serial_Init()~"));
	printf("\nreboot=%04x %04x",reboot_check1,reboot_check2);//peter
	printf("\n\r%s",FW_CODE_VERSION);     
...
//sorbica add WDT reset here
		...
		//peter force to update the program
		key=Serial_Polling_Rx();
		
		if((count >= 3) || (key=='f'))
		{
			printf("\Peter FlashCheck Error!");
