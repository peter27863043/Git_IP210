...
void UDP_RECEIVE()
{
	...
	// --------------------------------
    // UDP Data Capture for special purpose RX
    // --------------------------------
	/*
	if((NetUARTData.pTelnetData->pEEPROM_Telnet->op_mode==NU_OP_MODE_UDP_LIS) ||
	   (NetUARTData.pTelnetData->pEEPROM_Telnet->op_mode==NU_OP_MODE_UDP_NOR)){
		if(UartTxBuf.TotalLen!=0) {
			cmd=0;
			for(j=0;j<UartTxBuf.TotalLen;j++){
				k=UartTxBuf.ri+j;
				if(k>=UartRxBuf.MaxLen)
			   		k=k-UartRxBuf.MaxLen;	
				if((j==0) && (UartTxBuf.buf[k]=='T')) cmd++;
				if((j==1) && (UartTxBuf.buf[k]=='e')) cmd++;
				if((j==2) && (UartTxBuf.buf[k]=='s')) cmd++;
				if((j==3) && (UartTxBuf.buf[k]=='t')) cmd++;
			}
			if(cmd==4){
		   		printf("UDP_CMD UartTxBuf Len=%d\n",UartTxBuf.TotalLen);
		   		//Clear Buffer ,don't send data to RS232
		   		RingIncRi(UartTxBuf,UartTxBuf.TotalLen);		
				// add your code
			}   
		}	
	}
...
u8_t on_tcp_rx()
{
	...
	{ 
		...
		// --------------------------------
		// test TCP program for TCP Client Mode RX
		// --------------------------------		
		if(NetUARTData.pTelnetData->pEEPROM_Telnet->op_mode==NU_OP_MODE_CLIENT) {
			if(uip_conn->rport==NetUARTData.pTelnetData->pEEPROM_Telnet->TelentPort){			
				if(uip_len!=0){
					if((NetUARTData.pkt_rx_data_offset[0]=='T') &&
						(NetUARTData.pkt_rx_data_offset[1]=='e')&&
						(NetUARTData.pkt_rx_data_offset[2]=='s')&&
						(NetUARTData.pkt_rx_data_offset[3]=='t')) {

						//Drop the data, don't send data to RS232
						// add your code
		     			return 1;
					}
				}	
...
void application(void)
{
	...
	//u16_t current_data,old_data,jjj;
	//u8_t  ByteData,I2CAdd;
	...
	while (1)
	{
		...
		if(GetTVState()<eScaler_Init_Data){
			if(Delta_Time(start)>=6)  // 100 msec.
		       TVStateInitial();
		}
		else
			TVStateInitial();
