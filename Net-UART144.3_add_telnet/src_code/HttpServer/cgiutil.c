...
char *uitos(u16_t int_value)
{
	...
	//peter add the negtive number process
	if (int_value > 0x7fff) {
...
//		if((fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).pos==0)&&(fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).len==0)&&(fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).varid==0))
		...
		else
		{
			...
			{
				//printf("tag index=%d pos=%d\n",hs->tag_index,fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).pos); //peter test
				...
				switch(switch_varid)
				{
					...
					case 0xf1:
						...
						#ifdef	CGIUTIL_DEBUG 
//							dprintf("\n\r hs->tag_index_start=%x",hs->tag_index_start);
						#endif
						break;
					case 0xf2:
						hs->tag_flag=2;
						...
						{							
							...
							if(HS->cgi_func.func(0xf2,&tag_value)==ERROR)
							{
								...
								hs->row_num--;
								hs->tag_flag=1;//Jesse add
								#ifdef	CGIUTIL_DEBUG
//								dprintf("\n\r hs->table_flag=%x",hs->table_flag);
								#endif	
							}
							else
							{
					...
					case 0xf4:
							if(hs->table2_flag)
						{
							...
							if(HS->cgi_func.func(0xf4,&tag_value)==ERROR)
							{
								...
								hs->col_num--;
								#ifdef	CGIUTIL_DEBUG
//								dprintf("\n\r hs->table2_flag=%x",hs->table2_flag);
								#endif	
							}
							else
							{
...
//						dprintf("\n\r hs->tag_flag=%x",hs->tag_flag);
						...
						{									
							...
							{	
								#ifdef	CGIUTIL_DEBUG
								printf("\n\r IO call back function return OK!");
								#endif									
				...
				if(((uip_len+tag_value_len)<uip_conn->mss))//||(hs->html_current_pos!=hs->fsfile->datalen))
				{
					...
					if(hs->table_flag)
					{
//						hs->html_current_pos=fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).pos+fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).len;
					...
					{
//						hs->html_current_pos+=fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).len;
//						hs->dataptr+=fsop[HS->cur_fsop_idx].get_tag(hs->tag_index).len;
					}
					tag_mss_flag=1;									
	...
	//dprintf("\n\r hs->tag_index_start=%x",hs->tag_index_start);
	printf("\n\rCGIUTIL");
	printf("\n\r  filename =%s",hs->file_name);
//	dprintf("\n\r hs->tag_index_unack=%x",hs->tag_index_unack);
//	dprintf("\n\r hs->tag_index=%x",hs->tag_index);
//	dprintf("\n\r hs->row_num=%x",hs->row_num);
//	dprintf("\n\r hs->col_num=%x",hs->col_num);
	printf("\n\rCGIUTIL End");
	#endif
//	printf("\n\r hs->count_unack=%x",hs->count_unack);
//	printf("\n\r }");
	return 1;
}
#endif
