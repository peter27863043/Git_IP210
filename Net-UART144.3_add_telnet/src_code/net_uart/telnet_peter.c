...
//----------------------------------joe for CLI-----------------------------------
				...
				// debug message
				printf("FIFO data len=%d cmd_index=%d\n",UART_newdata_len,cmd_index);				
				// example command check
				if(UART_newdata_len>0){
				  for(i=0;i<UART_newdata_len;i++){
				     if (cmd_index == 4) break;
				    switch(cmd_index){
						case 0: if (UartRxBuf.buf[UartRxBuf.wi+i]=='T') {
								   cmd_index++;
								}
								else {
								   cmd_index=0;
								...
								break;
						case 1: if (UartRxBuf.buf[UartRxBuf.wi+i]=='e') {
								   cmd_index++;
								}
								else {
								   cmd_index=0;
								...
								break;
						case 2: if (UartRxBuf.buf[UartRxBuf.wi+i]=='s') {
								   cmd_index++;
								}
								else {
								   cmd_index=0;
								...
								break;
						case 3: if (UartRxBuf.buf[UartRxBuf.wi+i]=='t') {
								   cmd_index++;
								}
								else {
								   cmd_index=0;
				...
				// 4 is a completed test command
				// 1~3 is a partial testcommand
				printf("RX cmd=%d\n",cmd_index);
				if(cmd_index==4) {
				  printf("UART RX Test Command\n");
			      cmd_index=0;
				}
				else if (cmd_index==0){
				  // pass data to ethernet 
				  printf("RX FIFO len=%d\n",UART_newdata_len);
				  RingIncWi(UartRxBuf,UART_newdata_len);
				}
