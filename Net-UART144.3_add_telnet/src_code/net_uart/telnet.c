...
void telnet_connected_init()
{
	...
	{
		//peter 
		//printf("tcp connection is established!\n");
...
#endif //DEBUG_TEMP1
		//printf("ReadEth2UARTTxBuf wi=%d ri=%d\n",UartTxBuf.wi,UartTxBuf.ri); //peter
		if(1){
...
u16_t cmd_index,cmd_len,cmd_in;
u8_t tv_cmd_data[20];
...
//	if((UART_newdata_len>0) && (RingAvallibleSpace(UartRxBuf) > 1024)) // modified by james	
	if(UART_newdata_len>0) // modified by jesse
	{
		...
		{
			if(UART_newdata_len==255)TelnetData.UART_FIFO_Overflow++; 				//Peter trace & check the overvflow key fragment
			TelnetData.LastUartRxTimer=timercounter;
			if(RingWBDistance(UartRxBuf)>=UART_newdata_len)							// check the avaliable space of UartRxBuf 
			{
				UART_Receive_Multi(&UartRxBuf.buf[UartRxBuf.wi],UART_newdata_len);	// copy the rx232 data form FIFO to UartRxBuf
#ifdef SW_UART_LOOBACK
				{
					int i=0;
...
//----------------------------------joe for CLI-----------------------------------
				...
				// modify program to capture the rs232 for your application
				if(UART_newdata_len>0){
				  for(i=0;i<UART_newdata_len;i++){
				    ...
				    switch(cmd_index){
						...
						case 0: if (UartRxBuf.buf[UartRxBuf.wi+i]>='0') {
							       cmd_len=UartRxBuf.buf[UartRxBuf.wi+i]-0x30;  
								   cmd_in=1;
								   cmd_index=1;
								}
								break;
...
{	
	...
	else
	{
		...
		//peter 
		//printf("Telnet RexmitLen=%d\n",TelnetData.RexmitLen);
...
void ReSendUARTRx2Eth()
{
    ...
    // --------------------------------
	// test TCP program for TCP Client Mode 
	// --------------------------------
	...
	if(TelnetData.RexmitLen<=mss)
	{
		...
		//peter
		//printf("udp1 uip_len=%d\n",uip_len);
	}
	else
	{
		uip_len=len;
		//peter
		//printf("udp2 uip_len=%d\n",uip_len);
	}
	uip_slen=uip_len;
#ifdef MODULE_FLASH512
	if( (NetUARTData.pTelnetData->pEEPROM_Telnet->op_mode==NU_OP_MODE_UDP_LIS)||
...
int ReadEth2UARTTxBuf()
{
...
#endif
	{
		if(RingAvallibleSpace(UartTxBuf)>=uip_len)
		{
			...
			{
				//peter copy data to UartTxBuf
				//printf("copy data to UartTxBuf 1\n");
			...
			else
			{
				...
				//printf("copy data to UartTxBuf 2\n");
			}
		}
		else	
		{
	...
	//		if(timer0_tick>=46900)	//at about 10ms
			if(timer0_tick>=4690)
			{
				...
				{ f_auto_send=1; }{
					...
					//peter
					//if(m_Ack_Now!=m_Ack_Old){
					//	uip_conn=TelnetData.uip_conn_last;
					//	uip_process(TCP_DELAY_ACK);
					//	CheckPktSend(); 		
					//}
				}
