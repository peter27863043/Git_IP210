...
u8_t  m_mode_name[15];
u8_t  m_firmware_version[15];
u8_t  m_serial_number[15];
u16_t m_operation_time_H;
u16_t m_operation_time_L;
...
u8_t ssi_status(u8_t varid, data_value *vp)
{
	...
	switch(varid) 
	{
		...
		case CGI_STA_MODE:		vp->value.string =m_mode_name; 			break;
		case CGI_STA_SN:		vp->value.string =m_serial_number; 		break;
		case CGI_STA_FW_VER:	vp->value.string =m_firmware_version; 	break;
		
		case CGI_STA_PWR:		switch(m_pwr_data)
								{
									case 0		: vp->value.string = "Standby"; 	break;
									default		: vp->value.string = "Power On"; 	break;
								}
								break;

		case CGI_STA_SRC:		switch(m_src_data)
								{
									case 0		: vp->value.string = "HDMI1"; 		break;
									case 1		: vp->value.string = "HDMI2"; 		break;
									case 2		: vp->value.string = "HDMI3"; 		break;
									case 3		: vp->value.string = "Mode"; 		break;
									case 4		: vp->value.string = "VGA";   		break;
									case 5		: vp->value.string = "YPbPr";		break;
									default		: vp->value.string = "AV"; 			break;
								}
								break;
		
		case CGI_STA_OP_TIME_H:	if(m_operation_time_H==0)
									vp->value.string ="";
								else{
									vp->value.digits_3_int=m_operation_time_H;
									vp->type=digits_3_int;
								}	
								break;
								
		case CGI_STA_OP_TIME_L: vp->value.digits_3_int=m_operation_time_L;
								vp->type=digits_3_int;
								break;
		case CGI_STA_DNS1:
		case CGI_STA_DNS2:
		...
		case CGI_STA_DNS4:
			if(EEConfigInfo.DHCPEnable!=sDHCP_DISABLE) {
				if(DHCP_get_ip==1){
					pByte=(u8_t *)&DHCPCInfo.DNSAddr;
					eep_val=pByte[varid-CGI_STA_DNS1];
				}	
			}	
			else if (EE_Result == 1)
			{	EEPROM_Read_Byte(loc_EEDNSAddress + (varid-CGI_STA_DNS1), &eep_val);	}
			else
			{	eep_val = EEConfigInfo.DNSAddress[(varid-CGI_STA_DNS1)];	}
		...
		case CGI_STA_GW4:
			if(EEConfigInfo.DHCPEnable!=sDHCP_DISABLE) {
				if(DHCP_get_ip==1){
					pByte=(u8_t *)&DHCPCInfo.GetwayAddr;
					eep_val=pByte[varid-CGI_STA_GW1];
				}	
			}	
			else if (EE_Result == 1)
			{	EEPROM_Read_Byte(loc_EEGateway+ (varid-CGI_STA_GW1), &eep_val);	}
			else
			{	eep_val = EEConfigInfo.Gateway[(varid-CGI_STA_GW1)];	}
			vp->value.digits_3_int=eep_val;
			vp->type=digits_3_int;
			break;
		...
		case CGI_STA_SM4:
			if(EEConfigInfo.DHCPEnable!=sDHCP_DISABLE) {
				if(DHCP_get_ip==1){
					pByte=(u8_t *)&DHCPCInfo.SubnetMask;
					eep_val=pByte[varid-CGI_STA_SM1];
				}	
			}	
			else if (EE_Result == 1)
			{	EEPROM_Read_Byte(loc_EESubnetMask + (varid-CGI_STA_SM1), &eep_val);	}
			else
			{	eep_val = EEConfigInfo.SubnetMask[(varid-CGI_STA_SM1)];	}
			vp->value.digits_3_int=eep_val;
			vp->type=digits_3_int;
			break;
		...
		case CGI_STA_IP4:
			if(EEConfigInfo.DHCPEnable!=sDHCP_DISABLE) {
				if(DHCP_get_ip==1){
					...
					eep_val=pByte[varid-CGI_STA_IP1];
				}	
			}	
			else if (EE_Result == 1)
			{	EEPROM_Read_Byte(loc_EEIPAddress + (varid-CGI_STA_IP1) , &eep_val); }
			else
			{	eep_val = EEConfigInfo.IPAddress[(varid-CGI_STA_IP1)];	}
			vp->value.digits_3_int=eep_val;
			vp->type=digits_3_int;
			break;
