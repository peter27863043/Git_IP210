...
u8_t ssi_errormsg(u8_t varid, data_value *vp)
{
    switch (varid)
    {
        case CGI_TITLE :
			...
			if( errmsgflag == ERRORMSG_NOSERVICE )
				vp->value.string = "Can't Change Setting";
        ...
        case CGI_BNAME:
        	vp->value.string=" ok ";
        	if((errmsgflag == ERRORMSG_RESET)||(errmsgflag == ERRORMSG_SETOK)||(errmsgflag == ERRORMSG_NOSERVICE))
