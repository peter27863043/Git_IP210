...
#endif 
//end for UDP
#define UIP_APPCALL     http_server
#define HTONS htons
//For MUIP
#ifdef MODULE_MUIP
#include "muip.h"
...
u16_t uip_dnsaddr[2];
#ifdef MODULE_VLAN_TAG
u8_t xdata eth_vlan_buf[UIP_BUFSIZE+2+8];
u8_t *uip_buf_real=&eth_vlan_buf[4];
u8_t *uip_buf=&eth_vlan_buf[4];
...
#else
u8_t xdata uip_buf_real[UIP_BUFSIZE+2];
u8_t* uip_buf=uip_buf_real;
#endif
...
//#endif /* UIP_LOGGING == 1 */
/*-----------------------------------------------------------------------------------*/
void chg_uip_buf(u8_t* new_buf)
{
	uip_buf=new_buf;
	uip_appdata=(&uip_buf[UIP_LLH_LEN+40]);	
}
/*-----------------------------------------------------------------------------------*/
void restore_uip_buf()
{
	uip_buf=uip_buf_real;
	uip_appdata=(&uip_buf[UIP_LLH_LEN+40]);
}
/*-----------------------------------------------------------------------------------*/
void
...
*               
*******************************************************************************/
extern u8_t ResetTCP_Port;
extern u8_t TCP_Status;
void
uip_process(u8_t flag)
{
  ...
  if(flag == UIP_TIMER) {
	...
	// --------------------------------
	if(uip_connr->rport==TelnetData.pEEPROM_Telnet->TelentPort){
		TCP_Status=uip_connr->tcpstateflags;
		if((NetUARTData.pTelnetData->pEEPROM_Telnet->op_mode==NU_OP_MODE_CLIENT)&&
			(uip_connr->tcpstateflags == ESTABLISHED)){
			if(ResetTCP_Port==0xaa){
				printf("TCP Reset \n");
				ResetTCP_Port=0;
...
#endif
	    uip_flags = UIP_TIMEDOUT;
		if(uip_connr==TelnetData.uip_conn_client){
...
if(!lostcnt--)
{
 ...
 found:
  ...
  case ESTABLISHED:
	...
	if(uip_connr->tcpstateflags & UIP_STOPPED){	//Joe Fix Hardware Flowcontrol will send window size 0 ack
...
#ifdef MODULE_VLAN_TAG
  if( (eth_vlan==1)||( (NetUARTData.pTelnetData->t_vlan_flag==1)&&(NetUARTData.pTelnetData->uip_conn_last==uip_conn) ) )
  { BUF->tcpchksum = ~(uip_tcpchksum()); }
#endif
